...
func signedHeader(t *testing.T, chainID string, height int64, signers []*signing.Secp256k1Signer) *blockoracle.Header {
	t.Helper()
	...
	commit := blockoracle.Commit{
		Height:     height,
		Round:      0,
		BlockID:    h.BlockHash,
		Signatures: make([]blockoracle.CommitSig, 0, len(signers)),
	}
	for _, s := range signers {
		...
		require.NoError(t, err)
		commit.Signatures = append(commit.Signatures, blockoracle.CommitSig{
...
func TestVerifier_RejectsForeignSignatureInMultiSig(t *testing.T) {
	...
	require.NoError(t, err)
	foreignAddr, err := blockoracle.AddressBytes(foreign.PublicKeyBytes())
	require.NoError(t, err)
	h.Commit.Signatures = append(h.Commit.Signatures, blockoracle.CommitSig{
