...
// blockoracle.BlockOracle, so height-sync can mount it directly with
// server.Mount.
type Mock struct {
	cfg        MockConfig
	totalPower int64
	...
	history map[int64]*blockoracle.Header
...
func NewMock(cfg MockConfig) (*Mock, error) {
	...
	return &Mock{
		...
		history:      make(map[int64]*blockoracle.Header),
...
func (m *Mock) AdvanceOne() (*blockoracle.Header, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	return m.advanceLocked()
}

func (m *Mock) advanceLocked() (*blockoracle.Header, error) {
	var height int64
	var t time.Time
	if m.latest == nil {
...
func (m *Mock) signCommit(h *blockoracle.Header, height int64, t time.Time) ([]blockoracle.CommitSig, error) {
	canonical := blockoracle.CanonicalHeaderBytes(h)
	...
	sigs := make([]blockoracle.CommitSig, 0, len(m.cfg.Validators))
	for i, v := range m.cfg.Validators {
		...
		sigs = append(sigs, blockoracle.CommitSig{
...
func (m *Mock) Latest(_ context.Context) (*blockoracle.Header, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
	if m.latest == nil {
...
// return an error.
func (m *Mock) At(_ context.Context, height int64) (*blockoracle.Header, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *Mock) Prove(_ context.Context, path string, height int64) (*blockoracle.Proof, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
	h, ok := m.history[height]
	if !ok {
...
func (m *Mock) Subscribe(ctx context.Context, fromHeight int64) (<-chan *blockoracle.Header, error) {
	m.mu.Lock()
	if m.closed {
		m.mu.Unlock()
	...
	sub := &subscription{
		ch:   make(chan *blockoracle.Header, 16),
		from: fromHeight,
	}
	id := m.nextSub
	m.nextSub++
...
func (m *Mock) fanoutLocked(h *blockoracle.Header) {
