...
func TestMockObserver_DeterministicForSameSeed(t *testing.T) {
	...
	for i := 0; i < 4; i++ {
		...
		require.NoError(t, err)
		require.True(t,
			bytes.Equal(blockoracle.CanonicalHeaderBytes(ha), blockoracle.CanonicalHeaderBytes(hb)),
...
func TestMockObserver_Subscribe_FanOut(t *testing.T) {
	...
	drain := func(ch <-chan *blockoracle.Header) *blockoracle.Header {
		select {
		case h := <-ch:
			return h
...
func TestMockObserver_PowerWeighted_HeavyAlwaysSigns(t *testing.T) {
	...
	require.NoError(t, err)
	heavyAddr, err := blockoracle.AddressBytes(heavy.PublicKeyBytes())
	require.NoError(t, err)
