...
| H13h | ✅ `TestLogPlane_SequencerStampAboveFloorWindowRejected` + `TestTurnTracker_UserStampDoesNotDegradeOpenTurn` + `TestHeartbeat_TipBeyondFloorWindowCarriesFloor` | `heightsync`, `user` | User heartbeat/start above `F(m)+W_conf` ⇒ `INVALID(height_unbacked)`. Host stamps are not this cap. Honest compose carries `F` when the courier tip is beyond `F+W_conf`. The turn clock (`LogResidentHeight` / confirm-finish `stampHeight`) ignores user stamps, so a `1<<40` start cannot degrade an open turn. Review write-up: [`height-sync-pr-1584-review.md`](./height-sync-pr-1584-review.md) P1. |
...
| H109 | ⏳ `TestFloorIndex_L6GarbageCleansFloor` | `heightsync` / `state` | **Phase F.** After L6 `DEFERRED_FAIL` (or Strong) on a host-signed `(H, hash)` that became `F`, the floor is cleaned — rewind to the last L6-confirmed host pair, or freeze unaided raises that have not been oracle-checked — **without** `INVALID`-ing the diff. Until then `F` stays polluted; slash does not restore the clock (spec §14 *Garbage cleanup after L6*, attack 24f). |
...
| Floor poisoning — an implausibly high reference height no honest producer can clear (24b) | The claim never becomes logical time: past `W_conf` the floor follows only a height `Q` distinct signers hold, and a carry cannot corroborate, so a lone claimant moves it nowhere and is marked `FLOOR_OUT_OF_BAND`. Honest parties no longer repeat the pair either — an out-of-reach floor is omitted rather than carried — so it stays under one signature, where L6 and L4 already reach it. The diff itself still applies: L0 asks only `≥ F(m)`. A host-signed lie **inside** `W_conf` that does become `F` is 24f | ✅ H54, ✅ H55, ✅ H56, ✅ H13g (basis), ✅ H12 (edge attribution) |
| Dirty floor after L6 — a garbage pair within `W_conf` became `F`; L6 later fails it (24f) | L6 names the originator and does **not** rewind `F` (lagging oracles must not split). Producers carry until omit. **Phase F** MUST clean the floor without `INVALID` | ⏳ H109 |
