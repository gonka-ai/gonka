...
| `PG-STEP-RESUME` | Kill the updater after removing the v4 PostgreSQL container; rerun. | The pending step retains the source volume, candidate image and migration target; recovery verifies the committed challenge before preflight. | Real Docker migration: `deploy/join/update-devshard-e2e_test.sh`; saved-target recovery after config changes: `python3 deploy/join/updater-postgres_test.py`. |
