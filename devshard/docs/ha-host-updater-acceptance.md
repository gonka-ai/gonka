...
| `UPG-05` | Keep a long-lived request and an SSE stream open during an ordinary v5 rollout. Test the first nginx-to-HAProxy cutover separately in a declared maintenance window. | Ordinary rollout preserves both streams; first cutover drains accepted work before replacing the public listener. New requests route only through an admitted candidate. | Automated Docker test required: `deploy/join/update-devshard-regression_test.sh --gate UPG-05`. |
...
| `CAP-01` | For `R` replicas, `N` HA children per replica, pool size `P`, and overlap of two generations, configure one fewer than `R * (2 * N * (P + 2) + 5)` connections. | Preflight rejects before rollout and reports required/available/reserved capacity. The documented example for `R=2,N=3,P=4` requires at least 82 before external reserve. | Automated: `deploy/join/update-devshard_test.sh` and `python3 deploy/join/versiond-storage-check_test.py` (real PostgreSQL configured for 82 versus 81 usable slots). Overlap workload/exhaustion testing remains required by CAP-02. |
