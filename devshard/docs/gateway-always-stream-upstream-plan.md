...
Review findings on landed Steps 5–9: [gateway-always-stream-review-fixes.md](./gateway-always-stream-review-fixes.md)
...
| Escalation gating | **Step 11:** under `ForceUpstreamStreaming`, first-token / `attempt_failed` apply even when the client asked `stream:false`; legacy 140s reduced-`max_tokens` + no-content timers stay only for `!ForceUpstreamStreaming` |
...
- No change to what devshardd hashes or stores (deferred to Step 15, explicitly optional).
...
while `chatRequest.Logprobs` / `TopLogprobs` keep the client's ask
(`request_filters_parameters.go:578-583`, and the comment at `request_filters.go:20-29`).
Client-facing rewrite is boolean on that ask: omit logprobs entirely when the client did not
request them; when the client asked for logprobs but not `top_logprobs`, empty the
`top_logprobs` arrays; when the client asked for any `top_logprobs > 0`, **keep the forced
upstream width** (`completionapi.ForcedTopLogprobs` = 5). Do **not** truncate alternatives
down to the client's numeric `top_logprobs` — validation and the client see the same width
once tops were requested.
...
- fold via maps (not a typed field whitelist): concatenate string fragments for
  `delta.content`, `delta.reasoning`, `delta.reasoning_content`, `delta.refusal`,
- take the first non-empty `delta.role`, defaulting to `assistant`,
- merge `delta.tool_calls` by their `index`: first non-empty `id` / `type` /
  `function.name`, concatenated `function.arguments` fragments, first-writer for
  other tool-call keys,
- concatenate per-chunk `choices[].logprobs.content[]` into one `logprobs.content` array,
- keep the last non-null `finish_reason` and `stop_reason`,
- take `usage` from the last event that carries a non-null one,
- take `id` / `created` / `model` / `system_fingerprint` (and any other top-level
  / choice / message keys) first-writer-wins so provider extensions survive,
...
measured in Step 14, with the optional Step 15 as the fix.
...
(unbounded line buffering). Step 13 is an independent accounting correctness fix for OpenAI
`n > 1` (multi-choice) vs single-`MaxTokens` reservation. Steps 14–15 are rollout and optional
follow-up. Step 16 is a later gateway UX/efficiency follow-up: let a duplicate client request join
an in-flight stream. Step 17 adds reconnect / winner-continuity OTel spans once the observability
e2e branch has landed.
...
[gateway-attempt-reconnect-plan.md](./gateway-attempt-reconnect-plan.md) Step 7
...
**Landed.** Same-nonce reconnect before escalation, winner continuity, live-attach resume
cursor, protocol ≥ v5 gate, and citest (`TestAttemptReconnect_*`) are in. Default remains
`AttemptReconnectEnabled=false` until soak (reconnect-plan Step 9).

Still deferred in the linked plan (do not block marking this parent step done): soft-signal
persist (Step 6), Prometheus/dashboards/`skipped_protocol` (Step 8), default-on soak
(Step 9), cross-instance HA / ML reattach (Step 10), and LiveStream trim/lag metrics + hop
timestamps (Steps 5f–5g). Parent Step 17 covers reconnect OTel once
`ak/devshard-observability-e2e` merges.

With Steps 2 and 3 in place a broken attempt is *resumable*. Step 4 makes that path
fault-tolerant: try a **same-nonce reconnect to the same host** first, escalate only after a
bounded budget (default 1s), and keep the original attempt as the winner while it can still be
resumed — continuing from the delivered prefix rather than restarting the text.

**Protocol gate:** the reconnect code ships in the **gateway-v4** binary, but is applied only to
sessions bound to protocol **≥ `v5`**. On ≤v4 escrows the gateway keeps escalate-to-new-nonce
behavior.
...
`devshard/cmd/devshardctl/stream_rewrite.go`, `proxy.go`, `response_cache.go`.
...
1. In the streaming write path, drop a `chat.completion.chunk` event whose `choices` is empty
   ...
   (`shouldSuppressForcedUsageChunk`; cheap `"usage"` / `"delta"` / `"message"` gates, then
   parse). Synthesized usage from a host `chat.completion` rewrite honors the same intent.
2. `deferredWriter` carries `streamClientIntent` from request context alongside logprob intent.
3. Cache key hashes the full `streamClientIntent` (review [F5](./gateway-always-stream-review-fixes.md#f5--cache-key-covers-wantsstream-but-not-wantsusage)).

Tests: streaming client without `include_usage` sees no usage chunk; with `include_usage`
sees exactly one; content chunks with `"usage":null` are not dropped; aggregated JSON still
carries `usage`; four intent combinations produce distinct cache keys
(`stream_usage_suppress_test.go`, `force_upstream_stream_test.go`).
...
1. `streamingEscalationPolicy(params)` (`params.Stream || ForceUpstreamStreamingEnabled()`)
   drives `escalationForInflight`: first-token and `attempt_failed` escalation apply even when
   `params.Stream` (client ask) is false.
2. Under that policy, `awaitRace` does **not** arm the 140s reduced-`max_tokens` timer or the
   non-stream no-content timer — first-token + inter-chunk cover the same failure modes.
   `reducedMaxTokensParams` stays in the tree for one release if the flag is reverted.
   Legacy timers remain for `!ForceUpstreamStreaming && !params.Stream`.
   Note (reconnect plan Step 5e): no-content / max-attempt / hard-timeout defaults derive from
   protocol `ExecutionTimeout` (default 32m), not a standalone “30m” constant.
3. `longNonStreamEmptyFailureExempt` returns false under streaming escalation policy so empty
   SSE streams are not masked from the perf tracker.
4. **Deferred (separate commit):** wire configured `InterChunkStallTimeout` (default 60s) into
   the stall logic or delete it — still unused vs hardcoded `InterChunkStallLogThreshold` 30s.

**Tests** (`force_upstream_escalation_test.go`): first-token / attempt_failed triggers for
`stream:false` + force on; legacy skip when force off; empty host escalates without reduced
`max_tokens`; blocking host starts a secondary inside the first-token budget; exempt disabled
under force on. Existing `TestRunInference_NonStreaming*ReducedMaxTokens*` still cover the
legacy path with force off.
...
the SSE path between gateway and host, so the blast radius grows with Step 14's default-on.
...
This step can land before or after Steps 5–11; it should land **before** Step 14's default-on
flip so the soak does not widen the attack window.

### Step 13 — Multi-choice `n` vs single-`MaxTokens` reservation (accounting mismatch) ✅

Severity: **Low**. Closer to a limited accounting/correctness bug than a practical exploit or
high-impact DoS. Independent of `ForceUpstreamStreaming`, but D4's aggregator already groups by
`choices[].index` (`n > 1` is accepted via `chatRequest.N`), so always-stream must not pretend
multi-choice is free to leave unbounded relative to escrow.

#### Finding

Devshard accepts OpenAI-compatible `n` (up to 5) and forwards it for ML execution, while signed
reservation/settlement only account for a single `MaxTokens` output budget. When aggregate usage
exceeds that reservation, settlement caps the charge at `ReservedCost`. That is a real
execution-vs-accounting mismatch.

#### Important corrections / caveats

1. **Not automatic underpayment on all `n > 1` requests.**  
   Reservation uses `InputLength` as request **byte** length. Large prompts often over-reserve on
   input enough to cover aggregate `n ×` output tokens, so those requests bill normally.
   Undercharge is most relevant for small prompts + high `max_tokens` + `n > 1`.

2. **Gateway does not specially “handle” multi-choice as `n` billed inferences.**  
   It is one escrow inference; ML returns multiple `choices`; client streaming is pass-through of
   that one response/stream.

3. **Token counting vs charging are different.**  
   `usage.completion_tokens` may include aggregate choice tokens, but charge is still capped by
   the understated reservation when actual exceeds reserved.

#### Why severity is Low

1. **Weak attacker motive.** Clients almost always want one completion; paying (even partially)
   to generate `n=5` low-utility extras has little economic reason.
2. **User still pays; gateway retail > GNK token cost.** The requester consumes escrow/gateway
   quota and typically pays gateway markup above on-chain token economics — not free compute.
3. **Impact resembles inefficient resource use, not theft.** Executors/validators may do more
   work than settlement credits, but absolute inference GNK cost is currently negligible, and
   amplification is capped at `n ≤ 5`.
4. **Conditional and bounded.** Requires authenticated access, non-zero temperature (else `n`
   coerced to 1), and usually a reservation-tight prompt shape. Absolute effect is limited to
   the escrow pot / under-credited work.
5. **Not a practical DoS lever.** Any resource burn is paid for by the requester’s
   session/gateway spend.

#### Recommended fix

**Short term (landed):** PostLimits `ForceLiteralParameter{Value: 1, OverwriteOnly: true}` on
`n` in `request_filters_parameters.go` — same idiom as D1's `stream` force. Explicit `n > 1`
is rewritten to `1` on the wire; omitted `n` stays omitted (ML default is 1). Greedy
temperature-zero coercion remains as a prior rule (now redundant for `n` but still correct).

**Proper fix** (land with validation support for multiple choices — do not under-reserve on
chain while validation still assumes one choice):

- include `n` / aggregate output budget in signed start/receipt/record;
- reserve `(input + n × max_tokens)` (or equivalent);
- enforce that in host workload verification;
- make validation explicitly support multi-choice (or keep disallowing `n > 1` until it does).

#### Implementation notes (this plan)

1. Short-term landed independently of Steps 5–11 / 14 (`TestNormalizeChatRequestForcesSingleChoice`,
   `TestNormalizeChatRequestCapsThenForcesSingleChoice`). Aggregator (Step 5) `n: 2` fixtures, if
   any, stay as pure unit tests of chunk merging — they do not go through the gateway filter.
2. Proper fix is a protocol / validation change — track separately from the always-stream
   rollout; do not flip default-on (Step 14) in a way that expands multi-choice traffic without
   the short-term gate.
3. Remaining for the proper fix: reservation unit test that signed budget matches aggregate
   output; host/validation multi-choice support.

### Step 14 — Enable by default, soak, dashboards
...
### Step 15 — devshardd streaming-only (optional, after Step 14 is stable)
...
### Step 16 — In-flight chat stream join (optional, after Step 14)
...
Step 16 closes that gap for concurrent / retried identical clients:
...
### Step 17 — Reconnect / winner-continuity observability spans
...
| Client shape | `stream: false` client + streamed upstream → `application/json`; `stream: true` unchanged | **Unit tests cannot cover this** — see [Required e2e streaming coverage](#required-e2e-streaming-coverage-gates-step-14) |
...
| Multi-choice `n` | `n > 1` forced to 1 or rejected until reservation includes choice count | `cmd/devshardctl/request_filters_test.go` (+ reservation/validation when proper fix lands) |
...
### Required e2e streaming coverage (gates Step 14)

Every unit test for Steps 5–10 drives either `normalizeChatRequest` or a proxy
handler **directly**. Production traffic does neither: it enters
`handlePooledChat`, is normalized once by the gateway, and is then handed to
`newRuntimeMux(proxy)`, which normalizes the already-rewritten body a second
time. [R1](./gateway-always-stream-review3.md#r1--the-clients-stream-intent-is-destroyed-by-double-normalization)
is exactly the class of defect that gap hides: with `ForceUpstreamStreaming` on,
every non-streaming client was served SSE, `handleAggregated` never ran, and
Step 10's suppression inverted — and the whole unit suite stayed green.

The tests below therefore must exist and pass **through the real stack** before
Step 14 flips the flag on by default. They belong in `testenv/citest` next to
`gateway_chat_test.go`, guarded by `harness.SkipUnlessEnv(t, "TESTENV_CITEST")`,
booting with `harness.BootStack` and driving the gateway with
`harness.PostGatewayChatCompletion` / `PostGatewayChatCompletionStream`. Toggle
the flag the way `harness.PatchGatewayAttemptReconnect` does — `POST
/v1/admin/settings` with `{"redundancy":{"force_upstream_streaming":true}}` —
and run each assertion with the flag both **on and off**, since the flag is the
rollback lever and both sides must be correct.

| Test | Drives | Must assert |
|---|---|---|
| `TestGatewayForcedStreamClientShape` | `stream:false` and `stream:true` clients, flag on | The JSON client gets `Content-Type: application/json` and one parseable `chat.completion` with a non-empty `choices[0].message.content` — **never** an SSE body; the streaming client gets `text/event-stream` ending in `data: [DONE]`. Repeat with the flag off and require byte-comparable client-visible shape. |
| `TestGatewayForcedStreamUsageSuppression` | streaming client with and without `stream_options.include_usage`, flag on | Without `include_usage`: no client-visible chunk carries a top-level `usage`, and no chunk arrives that has `usage` and no `choices`. With it: exactly one final usage chunk, non-zero `prompt_tokens` / `completion_tokens`. The non-streaming client always gets `usage` in the aggregate. |
| `TestGatewayForcedStreamLogprobStrip` | clients with and without `logprobs`, flag on and off | A client that did not ask sees no `logprobs`, `top_logprobs`, `token_ids`, or `prompt_logprobs` anywhere in the streamed chunks or the aggregate. A client that asked for logprobs without `top_logprobs` gets `logprobs.content` with emptied `top_logprobs`. A client that asked for any `top_logprobs > 0` gets `logprobs.content` with the forced upstream width (5) — not truncated to the client's numeric ask. Catches intent inversion for logprobs, which the gateway forces regardless of the flag. |
| `TestGatewayForcedStreamAggregateMatchesUnforced` | same prompt / seed, flag on then off, `stream:false` | Identical `choices[0].message.content`, `finish_reason`, `model`, `object`; matching `completion_tokens`. This is the differential test the plan promised, run against the real devshardd rather than the mock alone. |
| `TestGatewayForcedStreamCacheIsolation` | identical body from a `stream:false` then a `stream:true` client, flag on | Each client gets its own shape; neither is served the other's cached body. Then repeat each request to confirm the second hit is served from cache **in the right shape**. |
| `TestGatewayForcedStreamFlagFlip` | flip the flag via admin between requests, including with one request in flight | Requests started before and after the flip each complete in the shape their own client asked for; no request ends with `stream:true` on the wire but no `stream_options` (the F3 hazard). |
| `TestGatewayAggregateSpillRoundTrip` | `stream:false`, large `max_tokens`, compose env `GATEWAY_AGGREGATE_MAX_MEMORY_BYTES` set small enough to force a spill | Complete, parseable JSON with content matching the unforced run; `devshardctl` logs `aggregate_spilled=true` for the request; the spool directory is empty afterward (files are unlinked at create and the fd is closed). |
| `TestGatewayAggregateOversizeAborts` | `stream:false` with `GATEWAY_AGGREGATE_MAX_RESPONSE_BYTES` below the real response size | A typed gateway error response, **never** a `200` carrying a truncated or half-folded body. |

Client-shape / usage / logprobs cover [review round 3](./gateway-always-stream-review3.md);
flag differential / cache / mid-flight flip cover the rollback lever; aggregate spill /
oversize exercise the F4 buffer end to end. Run via
...
| Risk | Mitigation |
|---|---|
| Aggregated JSON differs subtly from vLLM's own non-streaming output (tool calls, `stop_reason`, unusual fields) | Differential test in Step 9; single revertible flag; manual diff against a real vLLM node before Step 14 |
...
| Payload-store growth (D7) | Measured in Step 14; Step 15's optional aggregation is the fix |
| Larger upstream traffic from per-chunk forced logprobs / `top_logprobs: 5` | Measured in Step 14; the alternative is reducing `TopLogprobsForcedValue`, which is a validation decision, not a streaming one |
...
| Malicious executor grows unbounded SSE lines in `parseSSEResponse` | Step 12 hard-caps event size (default 1 MiB), aborts with typed error, counters; land before Step 14 default-on |
| Malicious executor streams unbounded *total* bytes into the aggregated buffer | Review fix [F4](./gateway-always-stream-review-fixes.md#f4--no-total-bytes-cap-on-the-aggregated-response): 2 MiB RAM / 16 MiB disk spool (`GATEWAY_AGGREGATE_MAX_*`), typed abort |
| Aggregator's field whitelist silently drops provider extensions (`delta.reasoning`, `reasoning_details`) | **Fixed** in [F1](./gateway-always-stream-review-fixes.md#f1--aggregator-drops-deltareasoning-and-other-provider-extensions) (map-based fold, unknown keys first-writer-wins) |
| Admin flip of `ForceUpstreamStreaming` mid-request yields `stream` without `include_usage` | **Fixed** in [F3](./gateway-always-stream-review-fixes.md#f3--forceupstreamstreaming-is-read-twice-per-request-from-a-racy-global) (atomic + per-request snapshot) |
| `n > 1` under-reserves vs aggregate choice tokens | Step 13 short-term force/reject `n = 1`; proper fix reserves `n × max_tokens` with multi-choice validation |
| In-flight join fans out a failed primary or wrong escrow attribution | Step 16: explicit disconnect policy + H6 attribution tests; optional until after Step 14 soak |
| Reconnect span work races the observability e2e branch | Step 17 waits for `ak/devshard-observability-e2e` merge; reuse its attempt/phase APIs only |
...
| The gateway normalizes, then the runtime proxy normalizes again, so intent re-derived from the forwarded body is the *forced* value | **Fixed** in [R1](./gateway-always-stream-review3.md#r1--the-clients-stream-intent-is-destroyed-by-double-normalization): the gateway pins `clientRequestIntent` in the request context and the proxy prefers it; e2e E1–E3 keep it honest |
| Spool exhaustion silently promotes every aggregated request from the 2 MiB memory budget to the 16 MiB wire budget | **Fixed** in [R3](./gateway-always-stream-review3.md#r3--degrading-to-ram-removed-the-memory-ceiling): degrades only while a share of `GATEWAY_AGGREGATE_MAX_DEGRADED_RAM_BYTES` is free, otherwise keeps the memory ceiling |
...
- [x] Step 4 — same-nonce reconnect before escalation, gated to protocol ≥ v5 ([separate plan](./gateway-attempt-reconnect-plan.md); core + citest landed; deferred there: Steps 5f–5g / 6 / 8–10)
- [x] Step 5 — `aggregateSSEStream` + unit tests
- [x] Step 6 — wire into `handleNonStreaming`, drop `assembleSSEChunks`
- [x] Step 7 — `streamClientIntent` + `chatRequest.StreamOptions`
- [x] Step 8 — cache key and capture read intent
- [x] Step 9 — force `stream` / `stream_options` upstream behind `ForceUpstreamStreaming`
- [ ] Steps 5–9 review fixes — F1–F4 gate Step 14 default-on ([fix plan](./gateway-always-stream-review-fixes.md)); F5 landed with Step 10
- [x] Step 10 — suppress forced usage chunk for streaming clients (+ F5 cache key)
- [x] Steps 5–10 review round 3 — R1 intent handoff and R3 degraded-RAM bound ([review](./gateway-always-stream-review3.md)); R2 / R4–R6 still open and gate Step 14
- [x] E2E streaming coverage (forced-stream client shape / usage / logprobs / differential / cache / flag flip / aggregate spill / oversize) — `make -C devshard/testenv citest-force-upstream-streaming`; [required before Step 14](#required-e2e-streaming-coverage-gates-step-14)
- [x] Step 11 — unify escalation on streaming timeouts (`InterChunkStallTimeout` wiring still a separate follow-up)
- [x] Step 12 — cap gateway SSE event size (`parseSSEResponse` unbounded `ReadBytes`)
- [x] Step 13 — multi-choice `n` vs single-`MaxTokens` reservation (short-term PostLimits force `n=1`; proper fix with validation still open)
- [ ] Step 14 — default on, soak, dashboards
- [ ] Step 15 — devshardd streaming-only deprecation (optional)
- [ ] Step 16 — in-flight chat stream join (optional; [proposal](./proposals/chat-stream-inflight-join.md))
- [ ] Step 17 — reconnect / winner-continuity OTel spans (after `ak/devshard-observability-e2e` merge)

## Related

- [proposals/always-stream-upstream.md](./proposals/always-stream-upstream.md) — the proposal
- [gateway-always-stream-review-fixes.md](./gateway-always-stream-review-fixes.md) — defects and
  optimizations found reviewing the landed Steps 5–9; F1–F4 gate Step 14, F5 lands with Step 10
- [gateway-always-stream-review2.md](./gateway-always-stream-review2.md) — second review pass over
  the landed F1–F10 fixes and Step 10
- [gateway-always-stream-review3.md](./gateway-always-stream-review3.md) — third pass: the gateway →
  proxy intent handoff (R1) and the aggregate buffer's degraded-RAM bound (R3), plus the open items
  that still gate Step 14
- [gateway-attempt-reconnect-plan.md](./gateway-attempt-reconnect-plan.md) — Step 4's reconnect and
  winner-continuity design (core landed; some observability / soak / HA steps deferred); parent
  Step 17 feeds that plan's Step 8 OTel work
...
- [proposals/chat-stream-inflight-join.md](./proposals/chat-stream-inflight-join.md) — Step 16:
...
- [proposals/chat-cache-attribution.md](./proposals/chat-cache-attribution.md) — response
  cache attribution, touches the same cache keyed in Step 8 / Step 16
- Branch `ak/devshard-observability-e2e` — gateway.attempt / phase spans that Step 17 extends
