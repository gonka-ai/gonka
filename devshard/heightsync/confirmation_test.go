...
func (o *stubConfirmOracle) Latest(context.Context) (*blockoracle.Header, error) {
	if o.err != nil {
		return nil, o.err
	}
	if o.stale {
...
func (o *stubConfirmOracle) At(context.Context, int64) (*blockoracle.Header, error) {
	return nil, context.Canceled
}

func (o *stubConfirmOracle) Prove(context.Context, string, int64) (*blockoracle.Proof, error) {
	return nil, context.Canceled
}

func (o *stubConfirmOracle) Subscribe(context.Context, int64) (<-chan *blockoracle.Header, error) {
...
func TestConfirm_QuorumThreshold(t *testing.T) {
	...
	or := &stubConfirmOracle{hdr: &blockoracle.Header{Height: 100, BlockHash: []byte{1}}}
...
func TestConfirm_StaleWhenOracleStale(t *testing.T) {
	or := &stubConfirmOracle{
		hdr:   &blockoracle.Header{Height: 10, BlockHash: []byte{1}},
		stale: true,
	}
	idx := heightsync.NewConfirmationIndex(heightsync.ConfirmationConfig{
		Roster: []string{"h1"},
...
func TestConfirm_FreshnessAndWindowEligibility(t *testing.T) {
	now := time.Unix(1_700_000_000, 0)
	const tip int64 = 11
	or := &stubConfirmOracle{hdr: &blockoracle.Header{Height: tip, BlockHash: []byte{1}}}
	idx := heightsync.NewConfirmationIndex(heightsync.ConfirmationConfig{
		Roster:        []string{"h1", "h2"},
		Quorum:        2,
...
func TestConfirm_CompactOnTipAdvance(t *testing.T) {
	now := time.Unix(1_700_000_000, 0)
	or := &stubConfirmOracle{hdr: &blockoracle.Header{Height: 50, BlockHash: []byte{1}}}
	idx := heightsync.NewConfirmationIndex(heightsync.ConfirmationConfig{
		Roster:        []string{"h1"},
		Quorum:        1,
		Freshness:     time.Hour,
...
func TestConfirm_MonotonicityAfterPrune(t *testing.T) {
	now := time.Unix(1_700_000_000, 0)
	const h int64 = 11
	or := &stubConfirmOracle{hdr: &blockoracle.Header{Height: h, BlockHash: []byte{1}}}
	idx := heightsync.NewConfirmationIndex(heightsync.ConfirmationConfig{
...
func TestConfirm_IndexUsesOriginatorNotCarrier(t *testing.T) {
	...
	idx := heightsync.NewConfirmationIndex(heightsync.ConfirmationConfig{
		Roster: []string{"h1", "h2", "h3"},
		Quorum: 3,
		Oracle: &stubConfirmOracle{hdr: &blockoracle.Header{Height: 10, BlockHash: []byte{1}}},
		Now:    func() time.Time { return now },
	})
...
func TestConfirm_LateOracleTipBelowH(t *testing.T) {
	...
	or := &stubConfirmOracle{hdr: &blockoracle.Header{Height: hOld, BlockHash: []byte{1}}}
	idx := heightsync.NewConfirmationIndex(heightsync.ConfirmationConfig{
		Roster: []string{"h1", "h2", "h3"},
		Quorum: 3,
		Oracle: or,
