...
func TestInboundTrust(t *testing.T) {
	...
	require.Equal(t, heightsync.TrustUntrustedPeer, heightsync.InboundTrust(hs, &blockoracle.Header{Height: 4}))
	require.Equal(t, heightsync.TrustPeerAligned, heightsync.InboundTrust(hs, &blockoracle.Header{Height: 5}))
	require.Equal(t, heightsync.TrustPeerAligned, heightsync.InboundTrust(hs, &blockoracle.Header{Height: 6}))
	require.Equal(t, heightsync.AttestationTrust(""), heightsync.InboundTrust(nil, &blockoracle.Header{Height: 10}))
}
