...
func buildSlotRecord(escrowID string, escrow *escrowBook, slotID uint32) SlotRecord {
	...
	for key, count := range escrow.counters {
		...
		case CounterChallenge:
			slot.UnresolvedChallenges += count
		case CounterInvalidTransition:
			slot.RecordedInvalid += count
		}
	}
...
func counterRecordLess(left, right CounterRecord) bool {
	...
	if left.Key.SlotID != right.Key.SlotID {
		return left.Key.SlotID < right.Key.SlotID
	}
	return counterSortKey(left.Key) < counterSortKey(right.Key)
}
