...
func (b *Book) applyProtocolTransition(event ProtocolTransition) error {
	return b.withEscrowEvent(event.EscrowID, event, func(escrow *escrowBook) error {
		...
		case ProtocolChallenged:
			...
			escrow.increment(CounterKey{Kind: CounterChallenge, SlotID: slotID})
			return nil
		case ProtocolValidated:
			...
			escrow.resolveChallenge(event.Nonce, slotID)
			return nil
		case ProtocolInvalidated:
			...
			escrow.resolveChallenge(event.Nonce, slotID)
			escrow.increment(CounterKey{Kind: CounterInvalidTransition, SlotID: slotID})
			return nil
		}
...
func (e *escrowBook) resolveChallenge(nonce uint64, fallbackSlot uint32) {
	slotID := fallbackSlot
	if challengedSlot, ok := e.challenges[nonce]; ok {
		slotID = challengedSlot
	}
	e.decrement(CounterKey{Kind: CounterChallenge, SlotID: slotID})
