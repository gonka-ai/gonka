...
func TestLifecycleReadyAndDrainStatus(t *testing.T) {
	lifecycle := newLifecycleState()
	e := buildServer(lifecycle)
	admin := buildAdminServer(lifecycle, func() bool { return true }, nil, func() bool { return true })
	e.GET("/work", func(c echo.Context) error {
...
func TestReadyReflectsStorageReadiness(t *testing.T) {
	...
	lifecycle.SetReady(true)
	storageReady := false
	admin := buildAdminServer(lifecycle, func() bool { return storageReady }, nil, func() bool { return true })
...
func TestReadyReflectsSessionRecoveryProgress(t *testing.T) {
	...
	lifecycle.SetReady(true)
	recovered := false
	admin := buildAdminServer(lifecycle, func() bool { return true }, nil, func() bool { return recovered })
