...
func (b *mockBridge) GetEscrow(_ uint64) (*bridge.EscrowInfo, error) {
	return b.escrow, nil
}
...
func (b *mockBridge) OnSettlementFinalized(_ uint64) error { return bridge.ErrNotImplemented }
func (b *mockBridge) SubmitDisputeState(_ uint64, _ []byte, _ uint64, _ map[uint32][]byte) error {
	return bridge.ErrNotImplemented
}
...
func populateStore(t *testing.T, store storage.Storage, numDiffs int) ([]types.SlotAssignment, *signing.Secp256k1Signer, *signing.Secp256k1Signer) {
	...
	sm, err := state.NewStateMachine("1", config, group, 100000, user.Address(), verifier)
	require.NoError(t, err)
