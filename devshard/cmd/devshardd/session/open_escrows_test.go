...
func TestHostManager_WarmEscrow_SoftFailsWhenNotInGroup(t *testing.T) {
	...
	mgr := NewHostManager(store, outsider, stub.NewInferenceEngine(), stub.NewValidationEngine(), nil, testutil.RuntimeTestVersion, br, nil, nil)
	t.Cleanup(func() { _ = mgr.Close() })
...
func TestHostManager_WarmEscrow_NoVersionClaimAcrossBoundVersions(t *testing.T) {
	...
	mgrA := NewHostManager(store, hosts[0], stub.NewInferenceEngine(), stub.NewValidationEngine(), nil, "v-a", br, nil, nil)
	mgrB := NewHostManager(store, hosts[0], stub.NewInferenceEngine(), stub.NewValidationEngine(), nil, "v-b", br, nil, nil)
	t.Cleanup(func() {
		_ = mgrA.Close()
		_ = mgrB.Close()
	})
