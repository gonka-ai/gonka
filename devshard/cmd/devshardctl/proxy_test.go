...
func setupTestProxyWithBalance(t *testing.T, numHosts int, engines []devshard.InferenceEngine, verifierAccept bool, balance uint64) *testProxyEnv {
	...
	for i := range hostSigners {
		sm, err := state.NewStateMachine("escrow-proxy", config, group, balance, userKey.Address(), verifier, testutil.MustMemoryStore(t, "escrow-proxy", userKey.Address(), config, group, balance))
		require.NoError(t, err)
	...
	userSM, err := state.NewStateMachine("escrow-proxy", config, group, balance, userKey.Address(), verifier, testutil.MustMemoryStore(t, "escrow-proxy", userKey.Address(), config, group, balance))
	require.NoError(t, err)
