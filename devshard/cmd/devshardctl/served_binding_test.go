...
//  3. Assert the threshold is crossed only for a mismatch or a missing served hash, including on an error stream.
func TestServedBindingFailureStrikesTheHost(t *testing.T) {
	for _, testCase := range []struct {
		...
		{name: "missing", verdict: user.ServedBindingMissing, wantStrike: true},
