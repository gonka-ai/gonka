...
// DefaultMaxSSEEventBytes line cap). Non-SSE shapes — {"events":[…]} envelopes
// and bare chat.completion JSON — remain as a bytes-only fallback for
// cache/replay and unit tests; handleAggregated never produces them.
func aggregateSSEStream(raw []byte, intent logprobClientIntent) []byte {
...
func aggregateSSEStreamReader(r io.Reader, intent logprobClientIntent) []byte {
	if r == nil {
...
func aggregateFromPayloads(payloads [][]byte, intent logprobClientIntent) []byte {
	if len(payloads) == 0 {
		return []byte(noResponseDataJSON)
	}
...
func filterPassthroughPayload(p []byte, intent logprobClientIntent) []byte {
...
func newCompletionFolder(intent logprobClientIntent) *completionFolder {
...
func foldCompletionChunks(payloads [][]byte, intent logprobClientIntent) ([]byte, bool) {
...
func decodeChoiceForFold(chRaw json.RawMessage, intent logprobClientIntent) (map[string]any, json.RawMessage, error) {
	var raw map[string]json.RawMessage
