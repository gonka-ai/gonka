...
func TestGatewayAccountingRecordsGhostDispatch(t *testing.T) {
	...
	for _, record := range records {
		...
		for _, counter := range record.Counters {
			if counter.Key.Disposition == accounting.DispositionGhost {
	...
	require.Equal(t, accounting.NoSendPoCUnavailable, ghosts[0].Key.NoSendReason)
	require.Equal(t, "none", ghosts[0].Key.DetailReason,
		"a recognized no_send_reason must not be duplicated into detail_reason")
	require.Equal(t, accounting.QuarantineNone, ghosts[0].Key.QuarantineMode)
	require.Equal(t, accounting.PhaseNormal, ghosts[0].Key.DispatchPhase)
}
