...
func TestHandleAggregatedPath_SpilledBodyFoldsViaOpenReader(t *testing.T) {
	...
	require.NoError(t, err)
	assembled := aggregateSSEStreamReader(src, logprobClientIntent{})
	require.JSONEq(t, string(aggregateSSEStream(wantBody.Bytes(), logprobClientIntent{})), string(assembled))
