...
// so the gateway can derive set_new_validators / next_poc_start from chain
// EpochInfo without depending on decentralized-api's computed stages.
const (
...
// deriveEpochPhaseFromParams mirrors EpochContext.GetCurrentPhase for cases
// where the chain EpochInfo response does not yet include phase.
