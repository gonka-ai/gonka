...
// TestUser_ForceHeightSyncTurn_AppearsOnlyInTriggerDiff locks the
// "single message in the chain" invariant for the manual-force flow
// (plan §5.5, status doc Step 9 test list).
//
// The user composer MUST insert MsgForceHeightSyncTurn into exactly one
// diff per forced-turn lifetime — the trigger diff. Subsequent
// PrepareInference calls inside [StartNonce, EndNonce] MUST NOT add a
// second MsgForceHeightSyncTurn even when InferenceParams.ForceHeightSyncAnchor
// is set on every call. After the window closes, a fresh ForceHeightSyncAnchor
// flag opens a new turn.
