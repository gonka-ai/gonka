...
func newSessionTestStateMachine(
	t *testing.T,
	...
	sm, err := state.NewStateMachine(
		"escrow-1", config, group, balance, userAddr, verifier,
		testutil.MustMemoryStore(t, "escrow-1", userAddr, config, group, balance),
	)
...
func setupSessionWithOptions(t *testing.T, numHosts int, balance uint64, grace uint64, opts ...SessionOption) (*Session, []*signing.Secp256k1Signer, *signing.Secp256k1Signer) {
	...
	for i := range hosts {
		sm := newSessionTestStateMachine(t, config, group, balance, user.Address(), verifier)
	...
	userSM := newSessionTestStateMachine(t, config, group, balance, user.Address(), verifier)
...
func TestUser_HostError_StateConsistency(t *testing.T) {
	...
	for i := range hosts {
		...
		sm := newSessionTestStateMachine(t, config, group, 100000, userKey.Address(), verifier)
		engine := stub.NewInferenceEngine()
	...
	userSM := newSessionTestStateMachine(t, config, group, 100000, userKey.Address(), verifier)
...
func setupDeadHostSession(t *testing.T, numHosts, aliveCount int, balance uint64, grace uint64) *Session {
	...
	for i := range clients {
		if i < aliveCount {
			sm := newSessionTestStateMachine(t, config, group, balance, userKey.Address(), verifier)
	...
	userSM := newSessionTestStateMachine(t, config, group, balance, userKey.Address(), verifier)
	session, err := NewSession(userSM, userKey, "escrow-1", group, clients, verifier,
