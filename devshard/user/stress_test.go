...
func runStress(t *testing.T, numHosts, rounds int) {
	...
	srStart := time.Now()
	_, err = state.ComputeStateRoot(preFinSt.Balance, preFinSt.HostStats, preFinSt.Inferences, preFinSt.Phase, preFinSt.WarmKeys, preFinSt.Fees, types.HeightSyncEscrowCommitFromState(&preFinSt))
	require.NoError(t, err)
