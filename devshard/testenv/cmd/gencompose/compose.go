...
func writeCompose(cfg *config.File, outPath string) error {
	...
	// A close failure here is the write failing to reach disk, so it has to reach the caller.
...
func writeEnvFile(cfg *config.File, outPath string) error {
	...
	fmt.Fprintf(&b, "TESTENV_CHAIN_ID=%s\n", cfg.ChainID)
	fmt.Fprintf(&b, "TESTENV_USER_PRIVATE_KEY=%s\n", cfg.User.PrivateKeyHex)
	fmt.Fprintf(&b, "TESTENV_ADMIN_API_KEY=%s\n", config.DefaultAdminAPIKey)
	fmt.Fprintf(&b, "TESTENV_KEYRING_PASSWORD=%s\n", cfg.Versiond.KeyringPassword)
...
func inVersiondPool(cfg *config.File, h config.HostCfg) bool {
	...
	return slices.Contains(strings.Fields(versiondHosts(cfg)), h.ID)
}
