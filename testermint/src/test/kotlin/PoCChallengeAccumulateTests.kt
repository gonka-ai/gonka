...
class PoCChallengeAccumulateTests : TestermintTest() {
    @Test
    fun `double-window challenge accumulates and normalizes to ordinary weight`() {
        logSection("=== TEST: PoCChallenge accumulate then normalize ===")
        ...
        val expectedCw = expectedNormalizedWeight(20, duration, stagePlusExchange(genesis))
        Logger.info(
            "Locked payment P=$locked epoch=${open.epochIndex} start=${open.startHeight} " +
                "finish=${open.finish} duration=$duration expectedCw=$expectedCw"
        )
        ...
        val first = waitForChallengeCommit(genesis, target)
        assertThat(first.startHeight).isEqualTo(open.startHeight)
        assertThat(first.commits.first().count).isEqualTo(10)

        requireEmitWindow(first, genesis.getCurrentBlockHeight())
        logSection("Emitting a second batch on the target")
        join1.emitPocV2Batch()
        val accumulated = waitForChallengeCommitCount(
            genesis,
            target,
            count = 20,
            startHeight = open.startHeight,
            deadline = open.finish - 1,
        )
