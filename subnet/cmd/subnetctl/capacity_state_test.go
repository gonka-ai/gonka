...
func TestSubnetRuntimeLoadIsActivePerWeight(t *testing.T) {
	rt := &subnetRuntime{}
...
func TestReserveRuntimeForModelPrefersHigherWeightEscrow(t *testing.T) {
	hi := &subnetRuntime{id: "hi", model: "M"}
	hi.active.Store(true)
	lo := &subnetRuntime{id: "lo", model: "M"}
	lo.active.Store(true)

	g := NewGateway([]*subnetRuntime{lo, hi}, NewGatewayLimiter(0, 0), "M")
...
func TestReserveRuntimeForModelTreatsZeroWeightEscrowAsLastResort(t *testing.T) {
	healthy := &subnetRuntime{id: "healthy", model: "M"}
	healthy.active.Store(true)
	dead := &subnetRuntime{id: "dead", model: "M"}
	dead.active.Store(true)

	g := NewGateway([]*subnetRuntime{healthy, dead}, NewGatewayLimiter(0, 0), "M")
	g.capacity = NewCapacityState()
