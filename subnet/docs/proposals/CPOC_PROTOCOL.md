...
| **F — Tests & adversarial catalog**     | Confidence.                                                    | Table-driven tests covering **C1…C14** (including double-claim fraud **C2'** across the seal-window race, `ready`-outcome probes **C7'**, developer-withholding **C13**, and low-load strategic-delay / heartbeat mitigation **C14**) plus cryptographic edge cases (bad signature, wrong session, wrong epoch) and quiet-session cases (no intermediate `Diff` entries between `R_req` and `N_carry`, with and without heartbeat).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
- `Invalid` (local) — `V` emits `CPoCVote(Invalid, target, reason_code, …)` to the collector. Quorum of such votes settles the verdict and produces the evidence bundle for finalization. Applies to C2, **C2'** (double-claim, Invalid against host), C4, strictly-confirmed C6, the causality-fail variant **C3'** (Invalid against carrier), and **C13** (Invalid against developer for withholding — collector is the next executor host, not `D`). For C2', the vote may be **superseding** if a contradictory `MsgConfirmStart` lands within the seal window after an initial `Valid` verdict; the collector de-duplicates on `(N_carry, V_pubkey)` and keeps only the latest-signed vote.
