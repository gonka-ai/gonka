...
The subnet proxy uses two independent systems to handle misbehaving hosts:
...
Authorization: Bearer $SUBNET_ADMIN_API_KEY
...
| `ReceiptTime` | Wall clock when `subnet_receipt` SSE event arrived |
