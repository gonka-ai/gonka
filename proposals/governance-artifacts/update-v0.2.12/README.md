...
v0.2.12 introduces a governance-controlled `FeeParams.min_gas_price_ngonka` enforced during both `CheckTx` and `DeliverTx`. The initial value is **10 ngonka per gas unit**, which works out to roughly **$0.00046** for a typical transaction. Negligible for legitimate users, but meaningful at spam volumes: flooding 100,000 transactions would cost an attacker around $46. Governance can adjust the parameter without a chain upgrade.
