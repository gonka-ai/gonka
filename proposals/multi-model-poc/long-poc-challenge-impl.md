...
`MsgSubmitPoCChallengeValidations` is `MsgSubmitPocValidationsV2` with `PocStageStartBlockHeight` = this segment's `start_height`. Validations are accepted from the completion of slice commits until settlement. Load the signer's eligible model set and guardian status from one snapshot read. Cache each target's challenge, segment, assigned models, counted slices, and commit readiness for the batch. Sample nonces across counted slices in proportion to count. Check each subset against that slice root. One vote covers the segment.
...
`module.go` `onEndOfPoCValidationStage`: `Finalize(epoch)`, then `SettleAccounts`. `Finalize` auto-approves segments shorter than 300 blocks. It fails on a missing counted commit, a failed slice weight check, a missing vote, or the `unrelated_leave` flag set by `removeFromEpochGroups`. A still-pending punishable segment is decided from the votes present then, or fails as no-vote. It writes `ConfirmationWeight` for a last punishable segment that was voted in this window. `SettleAccounts` executes normally using the current participant statuses.

`SettleAccounts` pays challenges inside its existing cache after the settle writes and before commit. Only challenges for the settling epoch are paid. `gross_share` is `ParticipantFullWeight / TotalRewardWeight * minted amount` from that settle result. Pass: vest `P` to the target with `PayParticipantFromModule`. Target-caused fail (rejected, underweight, missing commit): refund `P`, then vest `min(E, gross_share)` to the challenger from the minted leftover before the remainder goes to governance, and only if that target's settle result is zero work and zero reward. No-vote or unrelated leave: refund `P` only. A payout error rolls back settlement and keeps the challenge. Older challenges stay unpaid.
...
## Lifecycle
  ...
  --> Safety window: `Seal` the last segment --> leave `Generating` --> flush --> `InferenceUpAllCommand`. Last punishable segment votes in the PoC N+1 validation window. `Finalize` decides any still-pending segment --> `SettleAccounts` pays `P` and `min(E, gross_share)` in the same cache.
  --> Fail: write `ConfirmationWeight` / `ConfirmationPoCRatio` --> INACTIVE --> set `generating_end` --> leave `Generating`. Payment is inside `SettleAccounts`.
