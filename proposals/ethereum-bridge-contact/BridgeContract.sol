 ...
 * @dev Ethereum bridge contract for cross-chain token transfers using BLS threshold signatures
 * @notice This contract receives ERC-20 tokens and processes withdrawals signed by validators
 */
contract BridgeContract is Ownable, ReentrancyGuard {
    ...
    function withdraw(WithdrawalCommand calldata cmd) external nonReentrant onlyNormalOperation {
        ...
        // 3. Signature Verification: Use cached group key
        bytes32 messageHash = keccak256(
            abi.encodePacked(cmd.epochId, cmd.requestId, cmd.recipient, cmd.tokenContract, cmd.amount)
        );
