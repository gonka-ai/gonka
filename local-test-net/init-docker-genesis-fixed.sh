...
if [ -f "$GENESIS_FILE" ]; then
    echo "Chain already initialized, skipping init step"
else
...
# Create a key (skip if already exists)
...
if ! $APP_NAME keys show "POOL_product_science_inc" --keyring-backend $KEYRING_BACKEND --keyring-dir "$STATE_DIR" >/dev/null 2>&1; then
  $APP_NAME keys \
      --keyring-backend $KEYRING_BACKEND --keyring-dir "$STATE_DIR" \
...
if ! $APP_NAME keys show "$KEY_NAME_WARM" --keyring-backend $KEYRING_BACKEND --keyring-dir "$STATE_DIR" >/dev/null 2>&1; then
  $APP_NAME keys \
      --keyring-backend $KEYRING_BACKEND --keyring-dir "$STATE_DIR" \
      add "$KEY_NAME_WARM"
fi
...
# Get addresses
GENESIS_ADDRESS=$($APP_NAME keys show "$KEY_NAME" --address --keyring-backend $KEYRING_BACKEND --keyring-dir "$STATE_DIR")
POOL_ADDRESS=$($APP_NAME keys show "POOL_product_science_inc" --address --keyring-backend $KEYRING_BACKEND --keyring-dir "$STATE_DIR")
...
echo "Warm key address: $WARM_KEY_ADDRESS"

# Add genesis accounts (skip if already added)
if [ ! -f "$STATE_DIR/config/genesis-accounts-added.flag" ]; then
  MILLION_BASE="000000000000000000"
  MILLION_NATIVE="1000000000000000000${COIN_DENOM}"
  
  $APP_NAME genesis add-genesis-account "$GENESIS_ADDRESS" "160$MILLION_NATIVE" --keyring-backend $KEYRING_BACKEND || echo "Genesis account already exists, continuing..."
  $APP_NAME genesis add-genesis-account "POOL_product_science_inc" "160$MILLION_NATIVE" --keyring-backend $KEYRING_BACKEND || echo "Pool account already exists, continuing..."
  ...
  # Check if gentx already exists
  if [ ! -f "$STATE_DIR/config/gentx/"*gentx.json ]; then
    $APP_NAME genesis gentx "$KEY_NAME" "1000000000000000000${COIN_DENOM}" --chain-id "$CHAIN_ID" \
      --moniker "mynode" \
      --url "$URL_VALUE" \
      --ml-operational-address "$WARM_KEY_ADDRESS" \
      || {
      echo "Failed to create gentx (may already exist), continuing..."
    }
  else
    echo "Gentx already exists, skipping..."
  fi
  ...
  if [ "$INIT_TGBOT" = "true" ]; then
    ...
    $APP_NAME genesis add-genesis-account $TG_ACC "100$MILLION_NATIVE" --keyring-backend $KEYRING_BACKEND || echo "TGBot account already exists, continuing..."
  fi
  
  touch "$STATE_DIR/config/genesis-accounts-added.flag"
fi

modify_genesis_file() {
  local override_file=$1
  if [ -f "$override_file" ]; then
    echo "Applying genesis overrides from $override_file"
    jq -s '.[0] * .[1]' "$STATE_DIR/config/genesis.json" "$override_file" > "$STATE_DIR/config/genesis.json.tmp"
...
modify_genesis_file '/root/genesis_overrides.json'
...
# Enable and configure gRPC to be accessible from outside container
echo "Configuring gRPC server..."
sed -i.bak 's/address = "localhost:9090"/address = "0.0.0.0:9090"/' $STATE_DIR/config/app.toml
sed -i.bak 's/address = "127.0.0.1:9090"/address = "0.0.0.0:9090"/' $STATE_DIR/config/app.toml
...
cosmovisor run start || {
  echo "Cosmovisor start failed, idling the container..."
  tail -f /dev/null
}
