...
func InferenceKeeperReturningMocksWithLogger(t testing.TB, logger log.Logger) (keeper.Keeper, sdk.Context, InferenceMocks) {
	ctrl := gomock.NewController(t)
	bankKeeper := NewMockBookkeepingBankKeeper(ctrl)
	bankViewKeeper := NewMockBankKeeper(ctrl)
	accountKeeperMock := NewMockAccountKeeper(ctrl)
	...
	authzKeeper := NewMockAuthzKeeper(ctrl)
	upgradeKeeper := NewMockUpgradeKeeper(ctrl)
	keep, context := InferenceKeeperWithMockAndLogger(t, logger, bankKeeper, accountKeeperMock, validatorSet, groupMock, stakingMock, collateralMock, streamvestingMock, bankViewKeeper, authzKeeper, upgradeKeeper)
	keep.SetTokenomicsData(context, types.TokenomicsData{})
	genesisParams := types.DefaultGenesisOnlyParams()
	keep.SetGenesisOnlyParams(context, &genesisParams)
	mocks := InferenceMocks{
...
func InferenceKeeperWithMockAndLogger(
	t testing.TB,
	logger log.Logger,
