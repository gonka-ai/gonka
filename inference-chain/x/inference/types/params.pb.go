...
func (m *GenesisOnlyParams) GetGenesisEnhancementEnabled() bool {
	if m != nil {
		return m.GenesisEnhancementEnabled
	}
	return false
}
...
func (m *GenesisOnlyParams) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	if m.GenesisEnhancementEnabled {
		i--
		if m.GenesisEnhancementEnabled {
			dAtA[i] = 1
		} else {
...
func (m *GenesisOnlyParams) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 13:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field GenesisEnhancementEnabled", wireType)
			}
			var v int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.GenesisEnhancementEnabled = bool(v != 0)
		case 14:
			if wireType != 0 {
