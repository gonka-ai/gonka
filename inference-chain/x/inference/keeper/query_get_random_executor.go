...
func (k Keeper) getParticipantSetByModel(goCtx context.Context, epochGroup *epochgroup.EpochGroup) (map[string]map[string]bool, error) {
	// Check if we have model-specific epoch groups
	if epochGroup.GroupData.ModelEpochGroups != nil && len(epochGroup.GroupData.ModelEpochGroups) > 0 {
		// Use the model-specific epoch groups
		participantSetByModel := make(map[string]map[string]bool)

		for _, modelGroup := range epochGroup.GroupData.ModelEpochGroups {
			modelId := modelGroup.ModelId

			// Get the members of this model group
			if participantSetByModel[modelId] == nil {
				participantSetByModel[modelId] = make(map[string]bool)
			}

			// Add all members from the validation weights
			for _, weight := range modelGroup.ValidationWeights {
				if weight.Weight > 0 { // Only include members with positive weight
					participantSetByModel[modelId][weight.MemberAddress] = true
				}
			}
		}
		...
		// Fall back to the old method if no model-specific groups exist
		k.LogInfo("No model-specific epoch groups found, falling back to old method", types.EpochGroup)
