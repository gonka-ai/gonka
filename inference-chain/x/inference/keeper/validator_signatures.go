...
func (k Keeper) SetValidatorsSignatures(ctx context.Context, signatures types.ValidatorsSignatures) error {
...
func (k Keeper) GetValidatorsSignatures(ctx context.Context, height int64) (types.ValidatorsSignatures, bool) {
	store := runtime.KVStoreAdapter(k.storeService.OpenKVStore(ctx))
	key := validatorSignaturesFullKey(uint64(height))
	bz := store.Get(key)
	if bz == nil {
		return types.ValidatorsSignatures{}, false
	}

	var proof types.ValidatorsSignatures
