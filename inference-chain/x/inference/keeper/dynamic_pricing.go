...
func (k *Keeper) UpdateDynamicPricing(ctx context.Context) error {
	...
	if statsDuration > 50*time.Millisecond {
		k.LogWarn("GetSummaryByModelAndTime slow query", types.Performance,
			"durationMs", statsDuration.Milliseconds(),
			"windowDurationMs", windowDurationMillis)
	}
