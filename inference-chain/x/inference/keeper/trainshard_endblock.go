...
// ProcessTrainshardEndBlock processes shard expirations and pruning
...
func (k Keeper) expireTrainshards(ctx context.Context, height int64, limit int) {
	...
	for _, key := range due {
		...
		if err := k.closeTrainshard(ctx, &shard, types.TrainshardStatus_TRAINSHARD_STATUS_EXPIRED, height); err != nil {
...
// deferTrainshardExpiry moves an active shard expiry key to a later height
