...
func (m *MsgSubmitDealerPart) ValidateBasic() error {
	...
	for i, c := range m.Commitments {
		...
		if allZero {
			return errorsmod.Wrapf(sdkerrors.ErrInvalidRequest, "commitment[%d] must be non-zero", i)
		}
	}
	...
	for i, p := range m.EncryptedSharesForParticipants {
		if len(p.EncryptedShares) == 0 {
			return errorsmod.Wrapf(sdkerrors.ErrInvalidRequest, "encrypted_shares_for_participants[%d] must have at least one share", i)
		}
		if len(p.EncryptedShares) > MaxSlotIndices {
			return errorsmod.Wrapf(sdkerrors.ErrInvalidRequest, "encrypted_shares_for_participants[%d] too many shares", i)
		}
		for j, sh := range p.EncryptedShares {
			if len(sh) == 0 {
