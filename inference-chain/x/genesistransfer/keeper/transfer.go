...
// TransferVestingSchedule is kept for backward compatibility with existing tests
// New code should use TransferOwnership instead
func (k Keeper) TransferVestingSchedule(ctx context.Context, genesisAddr, recipientAddr sdk.AccAddress) error {
	// Validate addresses first (for backward compatibility with tests)
	if genesisAddr == nil {
		return errors.ErrInvalidAddress.Wrap("genesis address cannot be nil")
	}
	if recipientAddr == nil {
	...
	// Get the genesis account
	genesisAccount := k.accountKeeper.GetAccount(ctx, genesisAddr)
	if genesisAccount == nil {
		return types.ErrAccountNotFound.Wrapf("genesis account %s does not exist", genesisAddr.String())
	}

	// Check if it's a vesting account and get vesting info
	...
	// For standalone vesting transfer, also transfer the vesting coins
	memo := fmt.Sprintf("Genesis vesting schedule transfer from %s to %s", genesisAddr.String(), recipientAddr.String())
	if err := k.bankKeeper.SendCoins(ctx, genesisAddr, recipientAddr, vestingCoins, memo); err != nil {
		return errors.ErrInvalidRequest.Wrapf("failed to transfer vesting coins: %v", err)
	}

	return nil
}

// TransferLiquidBalances is kept for backward compatibility with existing tests
// New code should use TransferOwnership instead
func (k Keeper) TransferLiquidBalances(ctx context.Context, genesisAddr, recipientAddr sdk.AccAddress) error {
	// Validate addresses
	if genesisAddr == nil {
		return errors.ErrInvalidAddress.Wrap("genesis address cannot be nil")
	}
	...
	genesisBalances := k.bankView.GetAllBalances(ctx, genesisAddr)
	if genesisBalances.IsZero() {
		return types.ErrInsufficientBalance.Wrap("genesis account has no liquid balances to transfer")
	}
	...
	// Check if this is a vesting account (genesisAccount already retrieved above)
	var isVestingAccount bool
	if genesisAccount != nil {
		// Check if account implements vesting account interface by checking for vesting-specific methods
		if _, isPeriodicVesting := genesisAccount.(*vestingtypes.PeriodicVestingAccount); isPeriodicVesting {
			isVestingAccount = true
		} else if _, isContinuousVesting := genesisAccount.(*vestingtypes.ContinuousVestingAccount); isContinuousVesting {
			isVestingAccount = true
		} else if _, isDelayedVesting := genesisAccount.(*vestingtypes.DelayedVestingAccount); isDelayedVesting {
			isVestingAccount = true
		} else if _, isBaseVesting := genesisAccount.(*vestingtypes.BaseVestingAccount); isBaseVesting {
			isVestingAccount = true
		}
	}

	// Determine which coins to transfer based on account type
	var coinsToTransfer sdk.Coins
	if isVestingAccount {
		// For vesting accounts, transfer all coins (both liquid and vesting)
		// TransferVestingSchedule() will only set up the vesting structure without transferring coins
		coinsToTransfer = genesisBalances
	} else {
		// For non-vesting accounts, validate that all coins are spendable and transfer all
		spendableCoins := k.bankView.SpendableCoins(ctx, genesisAddr)
		if !spendableCoins.Equal(genesisBalances) {
			return types.ErrInvalidTransfer.Wrapf(
				"genesis account has non-spendable coins: total=%s, spendable=%s",
				genesisBalances.String(),
				spendableCoins.String(),
			)
		}
		coinsToTransfer = genesisBalances
	}

	// Skip transfer if no coins to transfer (can happen for vesting accounts with no liquid coins)
	if coinsToTransfer.IsZero() {
		k.Logger().Info(
			"no liquid coins to transfer",
			"genesis_address", genesisAddr.String(),
			"recipient_address", recipientAddr.String(),
			"is_vesting_account", isVestingAccount,
		)
		return nil
	}

	// Perform the atomic balance transfer with logging
	memo := fmt.Sprintf("Genesis account ownership transfer from %s to %s", genesisAddr.String(), recipientAddr.String())
	if err := k.bankKeeper.SendCoins(ctx, genesisAddr, recipientAddr, coinsToTransfer, memo); err != nil {
		return errors.ErrInvalidRequest.Wrapf("failed to transfer liquid balances: %v", err)
	}

	// Log the successful transfer
	k.Logger().Info(
		"liquid balance transfer completed",
		"genesis_address", genesisAddr.String(),
		"recipient_address", recipientAddr.String(),
		"transferred_amount", genesisBalances.String(),
	)

	return nil
}

// ValidateBalanceTransfer performs pre-transfer validation without executing the transfer
// This can be used to check transfer eligibility before attempting the actual transfer
func (k Keeper) ValidateBalanceTransfer(ctx context.Context, genesisAddr, recipientAddr sdk.AccAddress) error {
	...
	// Check if genesis account exists and has balances
	genesisAccount := k.accountKeeper.GetAccount(ctx, genesisAddr)
	if genesisAccount == nil {
		return types.ErrAccountNotFound.Wrapf("genesis account %s does not exist", genesisAddr.String())
	}
...
// GetTransferableBalance returns the amount of liquid balances that can be transferred from an account
func (k Keeper) GetTransferableBalance(ctx context.Context, addr sdk.AccAddress) (sdk.Coins, error) {
	if addr == nil {
		return nil, errors.ErrInvalidAddress.Wrap("address cannot be nil")
	}

	// Check if account exists
	account := k.accountKeeper.GetAccount(ctx, addr)
	if account == nil {
		return sdk.NewCoins(), nil // Return empty coins if account doesn't exist
	}

	// Get spendable coins (excludes locked/vesting)
	spendableCoins := k.bankView.SpendableCoins(ctx, addr)
	return spendableCoins, nil
}
