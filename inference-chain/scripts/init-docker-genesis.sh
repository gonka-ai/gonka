...
if [ "$GENESIS_RUN_STAGE" = "start" ]; then
  ...
  if [ "$GENESIS_INDEX" = "0" ]; then
    ...
    echo "Skipping gentx collection for non-zero GENESIS_INDEX=$GENESIS_INDEX"
    if [ ! -f /root/input-artifacts/genesis-final.json ]; then
      echo "Error: /root/input-artifacts/genesis-final.json does not exist. Exiting."
      exit 1
    fi

    cp /root/input-artifacts/genesis-final.json "$STATE_DIR/config/genesis.json"
  fi
fi
