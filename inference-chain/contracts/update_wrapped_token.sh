...
echo "Submitting gov proposal: register code id and migrate all wrapped tokens..."
...
PROPOSAL_FILE="proposal_update_wrapped_token_$CODE_ID.json"
      ...
      --arg deposit "${MIN_DEPOSIT_AMOUNT}${COIN_DENOM}" '
      {
        "messages": [
          ...
          },
          {
            ...
            "authority": $authority,
            "newCodeId": $code_id,
            "migrateMsgJson": $migrate_msg,
            "limit": 0
          }
        ],
        "deposit": $deposit,
        "title": "Upgrade wrapped-token and migrate instances",
        "summary": "Register new wrapped-token code id and migrate all instances"
      }' > "$PROPOSAL_FILE"
