	...
	return func(ctx context.Context, plan upgradetypes.Plan, vm module.VersionMap) (module.VersionMap, error) {
		{
			...
			err := configurator.RegisterMigration(types.ModuleName, 4, func(ctx sdk.Context) error {
				return SetGenesisModels(ctx, k)
			})
			if err != nil {
...
func SetGenesisModels(ctx context.Context, k keeper.Keeper) error {
	models, err := k.GetGovernanceModels(ctx)
	if err != nil {
		k.LogError("Failed to get governance models during upgrade", types.Upgrades, "error", err)
		return fmt.Errorf("failed to get governance models: %w", err)
	}
