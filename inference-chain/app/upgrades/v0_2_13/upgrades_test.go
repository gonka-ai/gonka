...
// TestSetDevshardEscrowParams_BackfillsPhase4Fields covers the six fields added
// in v0.2.13 (create_devshard_fee, fee_per_nonce, refusal_timeout,
// execution_timeout, validation_rate, vote_threshold_factor). On an existing
// chain these decode as zero and would fail Validate; the migration must seed
// canonical defaults so the post-upgrade params validate.
func TestSetDevshardEscrowParams_BackfillsPhase4Fields(t *testing.T) {
	k, ctx, _ := keepertest.InferenceKeeperReturningMocks(t)
	...
	// SetParams marshals without invoking Validate(); this simulates a v0.2.12
	// on-disk state where the new fields decode as zero.
	require.NoError(t, k.SetParams(ctx, params))
...
func TestSetDevshardEscrowParams_PreservesExistingPhase4Fields(t *testing.T) {
	k, ctx, _ := keepertest.InferenceKeeperReturningMocks(t)
...
// TestBackfillDevshardEscrowFees verifies that pre-v0.2.13 escrow rows (with
// zero create_devshard_fee and fee_per_nonce) are populated with the current
// DevshardEscrowParams values, while rows that already have a non-zero snapshot
// are left untouched.
