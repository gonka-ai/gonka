...
| Resource                   | URL                                                                                                                     |
| -------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| Validator dashboard (seed) | [http://xj7-5.s.filfox.io:19242/](http://xj7-5.s.filfox.io:19242/)                                                      |
| Chain binary (on-chain)    | [inferenced v0.2.16-testnet-5](https://github.com/product-science/race-releases/releases/tag/release/v0.2.16-testnet-5) |
...
```bash
export NODE_RPC=http://xj7-5.s.filfox.io:19242/chain-rpc/
export NODE=http://xj7-5.s.filfox.io:19242/
```
...
v0.2.16-testnet-5 inferenced    → cosmovisor/upgrades/v0.2.16 (required before first node start)
v0.2.16-testnet-5 decentralized-api → .dapi/cosmovisor/upgrades/v0.2.16 (required before first api start)
...
For the testnet variables use the information below (seed host `xj7-5.s.filfox.io`, API/RPC published as `:19242`, P2P as `:19241`):

```bash
# Warm ML operational key name (created on the server in step 14 — not the offline cold key).
export KEY_NAME="join-18222"
...
export PUBLIC_URL="http://xj7-5.s.filfox.io:19230"
export P2P_EXTERNAL_ADDRESS="tcp://xj7-5.s.filfox.io:19229"
...
export SEED_API_URL="http://xj7-5.s.filfox.io:19242"
export SEED_NODE_RPC_URL="http://xj7-5.s.filfox.io:19242/chain-rpc/"
# Seed P2P is the seed host's published CometBFT P2P port (18221 → 19241), not :5000.
export SEED_NODE_P2P_URL="tcp://xj7-5.s.filfox.io:19241"
export BEACON_STATE_URL="https://sepolia.checkpoint-sync.ethpandaops.io"
export DAPI_API__POC_CALLBACK_URL="http://172.18.114.132:9100"
export DAPI_CHAIN_NODE__URL="http://node:26657"
export DAPI_CHAIN_NODE__P2P_URL="http://node:26656"
export RPC_SERVER_URL_1="http://xj7-5.s.filfox.io:19242/chain-rpc/"
export RPC_SERVER_URL_2="http://xj7-5.s.filfox.io:19242/chain-rpc/"
export NODE_RPC_URL="http://127.0.0.1:26657"
export PORT="8080"
export INFERENCE_PORT="5050"
export KEYRING_BACKEND="file"

# ML node image — pin for Ampere/CUDA 12.9 hosts (e.g. filfox RTX 3090).
# Default compose may point at a newer CUDA 13 tag that will not run on this GPU.
export MLNODE_IMAGE="ghcr.io/gonka-ai/mlnode:3.0.12-post4"
...
- Use the seed **proxy** RPC path `.../chain-rpc/` on port **19242**, not `:26657` on the seed host.
- Seed **P2P** is `tcp://xj7-5.s.filfox.io:19241` (published CometBFT P2P). Do **not** use `:5000` — that port is refused on this seed.
...
# Pulls node/api/proxy plus the pinned MLNODE_IMAGE (3.0.12-post4).
docker compose -f docker-compose.yml -f docker-compose.mlnode.yml \
  -f docker-compose.env-override.yml pull
```
...
### 8. Download v0.2.16-testnet-5 binaries (node + API)

Use the **on-chain upgrade artifacts** from [race-releases](https://github.com/product-science/race-releases/releases/tag/release%2Fv0.2.16-testnet-5) `v0.2.16-testnet-5` — not the generic GitHub Gonka release alone. Run in `deploy/join`:

```bash
curl -fL -o inferenced-amd64.zip \
  "https://github.com/product-science/race-releases/releases/download/release%2Fv0.2.16-testnet-5/inferenced-amd64.zip"
curl -fL -o decentralized-api-amd64.zip \
  "https://github.com/product-science/race-releases/releases/download/release%2Fv0.2.16-testnet-5/decentralized-api-amd64.zip"

sha256sum inferenced-amd64.zip decentralized-api-amd64.zip
# Checked 2026-09-10 against the live race-releases assets (tag release/v0.2.16-testnet-5):
#   inferenced-amd64.zip:
#     3acd59a3f068d3a0dc4272b7c1fee642e3cf6824cd492136dc232a4cf4c678d9
#   decentralized-api-amd64.zip:
#     5d8ad335a57d738b9320d175d8d69cef6951f4f31bd3231f9095f61b76493494
# GitHub may re-publish the same tag with new digests; if checksums differ, still verify
# `inferenced version` (via Docker, below) prints v0.2.16-testnet-5 before installing.
...
ABCI Handshake App Info ... software-version=0.2.16-testnet-5
```
...
curl -sS "http://xj7-5.s.filfox.io:19242/chain-rpc/status" \
...
```bash
curl -sS "http://xj7-5.s.filfox.io:19242/v1/participants" \
...
- [ ] v0.2.16-testnet-5 installed under `.inference/cosmovisor/upgrades/v0.2.16/bin/inferenced`
- [ ] v0.2.16-testnet-5 DAPI under `.dapi/cosmovisor/upgrades/v0.2.16/bin/decentralized-api`
- [ ] `/v1/versions` shows **node and api** both `v0.2.16-testnet-5`
- [ ] `cosmovisor/current` → `upgrades/v0.2.16` **before** first `node` start
- [ ] `CHAIN_ID=gonka-testnet-4`, `COIN_DENOM=ngonka`, `TRUSTED_BLOCK_PERIOD=2000`
- [ ] Seed RPC URLs use `http://xj7-5.s.filfox.io:19242/chain-rpc/`
- [ ] `SEED_NODE_P2P_URL` uses `tcp://xj7-5.s.filfox.io:19241` (not `:5000`)
...
- [ ] `MLNODE_IMAGE=ghcr.io/gonka-ai/mlnode:3.0.12-post4` (Ampere/CUDA 12.9 hosts)
---
