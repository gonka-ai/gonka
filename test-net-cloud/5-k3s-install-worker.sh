# Get token by running the command on the k8s-control-plane
sudo cat /var/lib/rancher/k3s/server/node-token

# Connet to the worker and run
# Use the K3S_TOKEN obtained from the control plane.
...
curl -sfL https://get.k3s.io | K3S_URL=https://$CONTROL_PLANE_INTERNAL_IP:6443 \
    K3S_TOKEN="$K3S_TOKEN" \
    INSTALL_K3S_EXEC="--flannel-iface=ens4 \
    --node-ip $WORKER_INTERNAL_IP \
    --node-label 'nvidia.com/gpu=true'" sh - # Adding a label for GPU node
