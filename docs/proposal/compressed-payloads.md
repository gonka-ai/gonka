...
| Gateway | hashes every non-protocol line it received as the executor enveloped them (and a single relayed body bare, as it was stored), and matches them against either signed hash of the Finish that applies for that nonce (`Session.BindReceivedStream`). A mismatch strikes the host locally: the bytes already reached the client, and the gateway cannot prove what the host sent |
...
The Finish counts whether it came in the stream tail, another host's mempool or gossip, and whether it applies before or after the stream ends; a queued Finish that never applies — a forged signature, a malformed hash — is never judged. A stream is judged once it reached `[DONE]` or its `devshard_meta` tail, even if the transport failed afterwards; the host writes meta only after execution finished, so the answer it streamed is complete. A stream that reached neither is not judged, since an honest network cut leaves partial hashes. A waiting stream or Finish is kept for twice the execution deadline (`2 × (ExecutionTimeout + TimeoutBuffer)`), and a waiting stream is never dropped while its record is still pending or started, so a late Finish always meets it. One gap remains: an executor that drops `[DONE]` and closes before meta, then lands its Finish through gossip, is not judged.

This closes both gaps the earlier gating opened, within the limits above: an executor that finishes one answer and streams another is struck, and an error miss is provable with the optimization on. Two error misses stay unprovable, as before: a JSON body relayed to a streaming gateway, and a reconnect replayed from the cached body. The executor stored a bare body there, while the gateway's error-miss payload is always an envelope; the binding check covers both shapes, the error-miss proof does not.
