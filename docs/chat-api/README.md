...
| `tools` | array | — | shape-bounded: function schema depth ≤16, nodes ≤256, branch arms ≤16, enum ≤256, size ≤64 KiB ([why](troubleshooting.md#reject-oversized-tool-schema)); `$ref`/`$defs`/`definitions` forbidden; `pattern` ≤512 B + regex compile; `function.name` ≤64 B; `tools[].function.strict` silent-stripped (vLLM parsers ignore) | [[OpenAI-1]](references.md#openai), [[CVE-2]](references.md#security-advisories), [[vLLM-47]](references.md#vllm) |
| `tool_choice` | string\|object | "auto" if tools | shape-strict when `tools` is present; `function.name` ≤64 B; `"required"` coerced ([why](troubleshooting.md#coerce-tool-choice-required)); stripped when `tools` is absent or empty ([why](troubleshooting.md#strip-tool-controls-without-tools)) | [[OpenAI-1]](references.md#openai), [[OpenAI-7]](references.md#openai), [[vLLM-46]](references.md#vllm) |
| `parallel_tool_calls` | bool | — | must be a boolean when `tools` is present (else reject — [why](troubleshooting.md#reject-malformed-param-types)); stripped when `tools` is absent or empty ([why](troubleshooting.md#strip-tool-controls-without-tools)); otherwise pass-through | [[OpenAI-1]](references.md#openai), [[vLLM-46]](references.md#vllm) |
