...
- Announcements: [gonka.ai/docs/network-updates](https://gonka.ai/docs/network-updates/) (through 2026-09-15)

> **Image tag ≠ running binary.**  
> `node` and `api` images may say `0.2.15` / `0.2.15-post5`, but Cosmovisor `current` decides what actually runs.  
...
| `api`       | `ghcr.io/product-science/api:0.2.15-post5`                  | Yes                      | DAPI; **binary via Cosmovisor** (below)           |
...
| `mlnode-*`  | `ghcr.io/gonka-ai/mlnode:3.0.16`                            | Yes                      | POC participation and inference serving           |
...
| `v4.1`       | `/devshard/v4.1/...` | [release/devshard/v4.1.0/devshardd.zip](https://github.com/gonka-ai/gonka/releases/download/release%2Fdevshard%2Fv4.1.0/devshardd.zip) |

- **v1 / v2** removed from `approved_versions` (earlier governance).
- Under the name `v4`, the binary actually running is currently `v4.1.0`-era **`v4.0.3`**: a patch inside a version keeps the name and only changes the sha256, so `approved_versions` still points at the v4.0.1 URL.
