...
# and saves the key details (including mnemonic) for the user.
...
# 2. Create the cold key and save the complete output to mnemonic.txt
echo "Creating key and saving details to mnemonic.txt..."
# 2>&1 redirects stderr to stdout, so we capture everything and prevent interactive prompts.
printf "%s\n%s\n" "$KEYRING_PASSWORD" "$KEYRING_PASSWORD" | $APP_NAME keys add "$KEY_NAME_COLD" --keyring-backend "$KEYRING_BACKEND" --home "$STATE_DIR" > "$STATE_DIR/mnemonic.txt" 2>&1
...
# 4. Copy the consensus private key to the root for easier access in Stage 5.
cp "$STATE_DIR/config/priv_validator_key.json" "$STATE_DIR/"


echo "---"
echo "Key generation complete."
echo "Your key details and mnemonic phrase are in: $STATE_DIR/mnemonic.txt"
