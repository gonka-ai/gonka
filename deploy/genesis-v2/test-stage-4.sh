...
for i in $(seq 1 $(($NUM_VALIDATORS - 1))); do
  ...
  NODE_ID_FILE="$BASE_DIR/genesis-0/artifacts/node_id.txt"

  if [ -f "$NODE_ID_FILE" ]; then
    # default local addressing via host.docker.internal and derived port
    GEN0_P2P_PORT=$((26656 + 0 * 10))
    CONFIG_p2p__persistent_peers="$(cat "$NODE_ID_FILE")@host.docker.internal:${GEN0_P2P_PORT}"
    export CONFIG_p2p__persistent_peers
