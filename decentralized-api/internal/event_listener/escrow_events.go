...
// escrowQuerier supplies slot membership for create/settle filtering.
type escrowQuerier interface {
	GetEscrowSlots(escrowID string) ([]string, error)
}
...
func (el *EventListener) localHoldsEscrowSlot(escrowID string) (holds bool, resolved bool) {
	...
	slots, err := el.escrowQuery.GetEscrowSlots(escrowID)
	if err != nil {
		return false, false
	}
