...
func TestIsRetryableDuplicateScanError(t *testing.T) {
	cases := []struct {
		...
		{"http 400", errors.New("proof request failed with status 400: root_hash does not match"), true},
		{"http 404", errors.New("proof request failed with status 404: not found"), true},
