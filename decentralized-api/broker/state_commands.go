...
func (c StartPocCommand) Execute(b *Broker) {
	...
	// Check if we should run PoC (regular OR confirmation OR challenge generation)
	shouldRunPoC := epochState.CurrentPhase == types.PoCGeneratePhase
	if overlayGeneratingChallengeWork(epochState) != nil {
		shouldRunPoC = true
	}
	underChallenge := overlayNothingPreserved()
	...
	for _, node := range b.nodes {
		...
		} else if !underChallenge && node.State.ShouldContinueInference() {
...
func (c StartPocCommand) shouldMutateState(b *Broker, epochState *chainphase.EpochState) bool {
	...
	for _, node := range b.nodes {
		...
		if !overlayNothingPreserved() && node.State.ShouldContinueInference() {
...
func (c InferenceUpAllCommand) Execute(b *Broker) {
	...
	if overlayGeneratingChallengeWork(epochState) != nil {
		logging.Info("InferenceUpAllCommand: no-op while generating challenge work", types.PoC,
			"current_block_height", epochState.CurrentBlock.Height)
		return
	}
