...
emit_host_fields() {
	...
	if [[ -n "$sha" ]]; then
		emit source_line "Source: gonka $sha (use attached Source code.zip / Source code.tar.gz, not the race-releases autogenerated archives)."
	else
		emit source_line "Source: gonka (use attached Source code.zip / Source code.tar.gz, not the race-releases autogenerated archives)."
	fi
}
...
classify_dispatch() {
	...
	if $want_host; then
		...
		if [[ -n "$github_sha" ]]; then
			emit source_line "Source: gonka $github_sha (use attached Source code.zip / Source code.tar.gz, not the race-releases autogenerated archives)."
		else
			emit source_line "Source: gonka (use attached Source code.zip / Source code.tar.gz, not the race-releases autogenerated archives)."
		fi
	else
...
archive_source() {
	...
	while [[ $# -gt 0 ]]; do
		...
		--output-dir)
			...
			;;
		*)
			die "unknown archive argument: $1"
			;;
		esac
	done
	...
	git -C "$root" archive --format=zip --prefix=gonka/ -o "$output_dir/Source code.zip" HEAD
	git -C "$root" archive --format=tar.gz --prefix=gonka/ -o "$output_dir/Source code.tar.gz" HEAD
}
